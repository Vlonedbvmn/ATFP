...
if __name__ == "__main__":
    ...
    if st.session_state.clicked:
        st.markdown(
            "### Ви обрали тестувальні дані. Це тестові набори даних, за допомогою яких ви можете ознайомитись з функціоналом проєкту та зрозуміти, яка модель підійде найкраще.")
    ...
    if st.session_state.clicked2:
        ...
        if uploaded_file is not None:
            ...
            st.button(label="Підтвердити", key="submit_own", on_click=submit_data,
                      args=(dataframe, option, option2, uploaded_file.name, means[fr]))
