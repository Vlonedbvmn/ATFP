...
if __name__ == "__main__":
    ...
    if st.session_state.df is not None:
        ...
        model = st.selectbox("Оберіть модель для передбачення",
        ...
        try:
            if model == "KAN":
                ...
                st.button(label="Підтвердити", key="kan", on_click=submit_data_KAN,
                          args=(ds_for_pred, iter, horizon, st.session_state.freq, inp))
        except Exception as ex:
            print(ex)
            st.warning('Надано не коректні гіперпараметри', icon="⚠️")
        ...
        try:
            if model == "Авто-вибір":
                ...
                st.button(label="Підтвердити", key="kan", on_click=submit_data_auto,
                          args=(ds_for_pred, iter, horizon, st.session_state.freq))
        except:
            st.warning('Надано не коректні гіперпараметри', icon="⚠️")
        st.divider()
